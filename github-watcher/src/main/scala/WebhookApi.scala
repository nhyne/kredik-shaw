...
object WebhookApi {
  ...
  private val fooBar: HttpApp[Console with Blocking with Namespaces, HttpError] = HttpApp.collectM {
    case req@Method.POST -> `apiRoot` => handlePostRequest(req).mapBoth(cause => HttpError.InternalServerError(cause = Some(new Throwable(cause.toString))), body => Response.text(body.toString))
  }

  val server: Server[Console with Blocking with Namespaces, HttpError] = Server.port(PORT) ++ Server.app(fooBar) ++ Server.maxRequestSize(100 * 1024)
  ...
  private def openedEvent(event: PullRequestEvent) = for {
    mergeSuccessful <- gitCloneAndMerge(event.pullRequest.base.repo.owner.login, event.pullRequest.head.repo.name, event.pullRequest.head.ref, event.pullRequest.base.ref)
    ns <- createPRNamespace(event.pullRequest.number, event.pullRequest.base.repo.name)
    applied <- applyFile(mergeSuccessful, ns)
    _ <- cleanupTempDir(mergeSuccessful)
  } yield applied

  private def gitClone(repo: String, branch: String) = Command("git", "clone", repo, s"--branch=$branch")
  ...
  private def applyFile(repoDir: File, namespace: Namespace) = for {
    exitCode <- Command("kubectl", "apply", "-n", namespace.metadata.flatMap(_.name).getOrElse(""), "-f", s"${repoDir.getAbsolutePath}/.watcher.conf/basic.yaml").run
  } yield exitCode

  // TODO: Should do this work in a temp directory that gets cleaned up later
  def gitCloneAndMerge(organization: String, repo: String, branch: String, target: String): ZIO[Blocking with Console, Throwable, File] = for {
    workingDir <- createRepoCloneDir(s"$organization-$repo")
    _ <- gitClone(s"https://github.com/$organization/$repo", branch).workingDirectory(workingDir).run
  ...
  final case class Repository(name: String, @jsonField("full_name") fullName: String, owner: Owner)
